			...
			while (true) {
				try {
					...
					for (BudgetCategory bc : document.getBudgetCategories()) {
						...
						// 
						for (String key : ((BudgetCategoryImpl) bc).getAmountOfBudgetPeriods().keySet()) {
							...
							if (splitKey.length == 2){
								...
								long amount = ((BudgetCategoryImpl) bc).getAmountOfBudgetPeriods().get(key);
								String newDateKey = 
								...
								newAmountMap.put(key, ((BudgetCategoryImpl) bc).getAmountOfBudgetPeriods().get(key));
							}
						}
						...
						if (changed){
							Logger.getLogger(ModelFactory.class.getName()).warning("Your data file has been updated to address bug #1811038.  Included inline are the details of the changes:\n\n"
									+ "New Map:\n" + newAmountMap + "\n\n"
									+ "Old Map:\n" + ((BudgetCategoryImpl) bc).getAmountOfBudgetPeriods());
